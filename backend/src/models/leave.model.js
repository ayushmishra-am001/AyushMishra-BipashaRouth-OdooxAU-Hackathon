...
// Admin/HR "Allocation" management (wireframe: Time Off | Allocation tabs).
// Updates the company-wide default for a leave type going forward. Existing
// employees who already have a balance row for the current year keep their
// already-allocated amount unless the caller also asks to sync it (see
// syncCurrentYearAllocations below) - changing the default shouldn't quietly
// take days away from someone who already used some of their old allocation.
async function updateDefaultAllocation(leaveTypeId, companyId, allocatedDays) {
  const result = await pool.query(
    `UPDATE leave_types SET default_allocation_days = $1
     WHERE id = $2 AND company_id = $3
     RETURNING id, name, default_allocation_days`,
    [allocatedDays, leaveTypeId, companyId]
  );
  return result.rows[0] || null;
}

// Applies the new default to every employee's *current-year* balance too,
// but only raises it - never lowers a balance below what's already been
// used, and never overwrites a balance that's already higher than the new
// default (e.g. a one-off manual top-up shouldn't be silently reverted).
async function syncCurrentYearAllocations(leaveTypeId, companyId, allocatedDays, year) {
  await pool.query(
    `UPDATE leave_balances lb
     ...
     WHERE lb.leave_type_id = lt.id
       ...
       AND lt.company_id = $3
       AND lb.year = $4
       AND lb.allocated_days < $1
       AND lb.used_days <= $1`,
    [allocatedDays, leaveTypeId, companyId, year]
  );
}
...
// True if the user already has a pending or approved request whose date
// range overlaps [startDate, endDate]. Rejected/cancelled requests don't
// block a new one for the same dates.
...
module.exports = {
  ...
  findTypeById,
  updateDefaultAllocation,
  syncCurrentYearAllocations,
